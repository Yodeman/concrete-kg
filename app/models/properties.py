...
class QuantitativeProperty(LLMExportableModel):
    ...
    min_value: Optional[float] = Field(
        default=None, description="Minimum acceptable value (for specifications)"
    )
    max_value: Optional[float] = Field(
        default=None, description="Maximum acceptable value (for specifications)"
    )
    uncertainty: Optional[float] = Field(
        default=None, description="Measurement uncertainty (± value in same units)"
    )
    test_method: Optional[str] = Field(
        default=None, description="Test method standard (e.g., 'ASTM C39')"
    )
...
class ProcessCondition(LLMExportableModel):
    ...
    unit: str = Field(..., description="Unit of measurement (e.g., '°C', '%', 'hours')")
    min_value: Optional[float] = Field(
        default=None, description="Minimum value for valid range"
    )
    max_value: Optional[float] = Field(
        default=None, description="Maximum value for valid range"
    )
    notes: Optional[str] = Field(
        default=None, description="Additional notes or requirements"
    )
...
class ExperimentRecord(LLMExportableModel):
    ...
    material_name: str = Field(..., description="Material being tested")
    date: Optional[str] = Field(
        default=None, description="Date of experiment (ISO format)"
    )
    conditions: list = Field(
        default_factory=list, description="List of ProcessCondition objects"
    )
    results: list = Field(
        default_factory=list, description="List of QuantitativeProperty results"
    )
    source: Optional[str] = Field(
        default=None, description="Source document or lab reference"
    )
    validated: bool = Field(
        default=False, description="Whether results have been validated"
    )
