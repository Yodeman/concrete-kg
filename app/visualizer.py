...
class Visualizer:
    ...
    def create_network_graph(
        ...
        if kg.graph.number_of_nodes() == 0:
            return self._create_empty_figure(
                width, height, "No data in knowledge graph"
            )
        ...
        fig = go.Figure(
            ...
            layout=go.Layout(
                ...
                legend=dict(
                    ...
                    bgcolor=(
                        "rgba(0,0,0,0.3)" if self.dark_mode else "rgba(255,255,255,0.8)"
                    ),
    ...
    def _compute_layout(
        self, graph: nx.DiGraph, algo: str
        ...
        try:
            if algo == "circular":
                ...
                return nx.spring_layout(
                    graph, k=2 / math.sqrt(graph.number_of_nodes()), iterations=50
                )
        except Exception:
    ...
    def create_component_heatmap(
        self, kg: KnowledgeGraph, width: int = 700, height: int = 500
    ) -> go.Figure:
        """
        ...
        if len(components) < 2:
            return self._create_empty_figure(
                width, height, "Not enough components for heatmap"
            )

        # Build adjacency matrix
        comp_ids = [c["id"] for c in components]
        comp_labels = [
            f"{c.get('label', '')} ({c.get('formula', '')})" for c in components
        ]
        ...
        fig.update_layout(
            ...
            yaxis=dict(
                tickfont=dict(color=self.text_color, size=10), autorange="reversed"
            ),
    ...
    def _create_empty_figure(self, width: int, height: int, message: str) -> go.Figure:
