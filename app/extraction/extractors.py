...
class LLMExtractor:
    ...
    def extract_compositions(self, text: str) -> List[MaterialComposition]:
        ...
        structured_llm = self.llm.with_structured_output(
            schema=_create_materials_schema(),
            method="json_schema"
        )
    ...
    def extract_substitutions(self, text: str) -> List[SubstitutionRelationship]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert in concrete materials science. Extract substitution relationships."),
            ("human", prompt_text)
        ])
        
        structured_llm = self.llm.with_structured_output(
            schema=_create_substitutions_schema(),
            method="json_schema"
        )
    ...
    def extract_synergies(self, text: str) -> List[SynergyRelationship]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", "You are an expert in concrete materials science. Extract synergy and antagonistic relationships."),
            ("human", prompt_text)
        ])
        
        structured_llm = self.llm.with_structured_output(
            schema=_create_synergies_schema(),
            method="json_schema"
        )
            ...
            "synergies": {
                ...
                "items": {
                    ...
                    "properties": {
                        ...
                        "effect_type": {"type": "string", "enum": ["SYNERGY", "ANTAGONISTIC"]},
                        "effect": {"type": "string"},
                        "strength": {"type": "number"},
                        "mechanism": {"type": ["string", "null"]}
                    },
                    "required": ["component1", "component2", "effect_type", "effect", "strength"]
                }
            }
        },
        "required": ["synergies"]
