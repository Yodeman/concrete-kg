...
class GraphRAG:
    ...
    def _ask_without_llm(self, question: str) -> Dict[str, Any]:
        ...
        for comp in components:
            if comp.get("formula", "").lower() in question_lower or \
               comp.get("label", "").lower() in question_lower:
                neighbors = self.kg.get_neighbors(comp["id"])
                results.append({
                    "component": comp["label"],
                    "formula": comp.get("formula"),
                    "related": [n.get("label", n.get("id", "Unknown")) for n in neighbors]
                })
    ...
    def _ask_with_memory_graph(self, question: str) -> Dict[str, Any]:
        ...
        prompt = ChatPromptTemplate.from_messages([
            ("system", """You are an expert in concrete materials science. 
...
Answer:""")
    ...
    def _build_context_for_question(self, question: str) -> str:
        ...
        materials = self.kg.get_nodes_by_type("Material")
        for mat in materials:
            if mat["label"].lower() in question_lower or \
               any(word in question_lower for word in mat["label"].lower().split()):
                neighbors = self.kg.get_neighbors(mat["id"])
                components = [n for n in neighbors if "percentage" in n or n.get("node_type") == "Component"]
                if components:
                    comp_strs = []
                    for c in components:
                        pct = c.get("percentage", "")
                        formula = c.get("formula", "")
                        comp_strs.append(f"{c['label']} ({formula})" + (f": {pct}%" if pct else ""))
                    context_parts.append(f"Material: {mat['label']}\nComponents: {', '.join(comp_strs)}")
        ...
        if not context_parts:
            ...
            for mat in materials[:5]:  # Limit to avoid context overflow
                ...
                components = [n for n in neighbors if n.get("node_type") == "Component"]
                if components:
                    comp_names = [f"{c['label']} ({c.get('formula', '')})" for c in components[:5]]
                    context_parts.append(f"Material: {mat['label']} contains: {', '.join(comp_names)}")
        
        return "\n\n".join(context_parts) if context_parts else "No relevant context found in knowledge graph."
        ...
        for nid, data in subgraph.nodes(data=True):
            if nid != node_id:
                ...
                try:
                    ...
                    paths = list(nx.all_simple_paths(self.kg.graph, node_id, nid, cutoff=2))
                    if paths:
                        path = paths[0]
                        # Get relationship
                        if len(path) >= 2:
    ...
    def get_composition_context(self, material: str) -> str:
        ...
        material_node = None
        for nid, data in self.kg.graph.nodes(data=True):
            if data.get("node_type") == "Material" and \
               material.lower() in data.get("label", "").lower():
                material_node = nid
                break
        ...
        if components:
            ...
            lines.append("|-----------|---------|------------|------|")
            for comp in sorted(components, key=lambda x: x.get("percentage", 0) or 0, reverse=True):
                pct = comp.get("percentage", "-")
                lines.append(f"| {comp['label']} | {comp.get('formula', '-')} | {pct}% | {comp.get('role', '-')} |")
    ...
    def search_by_property(self, property_query: str) -> List[Dict[str, Any]]:
        ...
        for source, target, data in self.kg.graph.edges(data=True):
            if data.get("relationship_type") == "AFFECTS":
                ...
                if query_lower in target_data.get("label", "").lower() or \
                   query_lower in desc:
