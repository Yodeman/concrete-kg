...
def render_sidebar():
    ...
    with st.sidebar:
        ...
        with st.expander("Neo4j Settings"):
            ...
            neo4j_user = st.text_input("Username", value=config.neo4j.username)
...
def extract_from_pdf(uploaded_file):
    ...
    with st.spinner("Extracting knowledge from PDF... This may take a minute."):
        try:
            ...
            st.success(
                f"Extracted {len(result.materials)} materials and {len(result.relationships)} relationships!"
            )
            st.rerun()
...
def render_welcome_screen():
    ...
    st.markdown("---")
    st.info(
        "👈 Click **Load Demo Data** in the sidebar to get started, or upload a PDF!"
    )
...
def render_extraction_results():
    ...
    with col4:
        st.metric(
            "📄 Source",
            (
                result.source_document[:30] + "..."
                if len(result.source_document) > 30
                else result.source_document
            ),
        )
...
def render_knowledge_graph():
    ...
    if viz_type == "Network Graph":
        ...
        fig = viz.create_composition_sunburst(
            kg, material_name=mat_name, width=700, height=700
        )
        st.plotly_chart(fig, use_container_width=True)
    ...
    if nodes:
        node_options = {
            f"{data.get('label', nid)} ({data.get('node_type', 'Unknown')})": nid
        ...
        with col2:
            ...
            for neighbor in neighbors:
                ...
                st.markdown(
                    f"- {rel} → {neighbor.get('label', neighbor.get('id', 'Unknown'))}"
                )
...
def render_graph_rag():
    ...
    if not config.llm.is_configured:
        st.warning(
            "⚠️ LLM not configured. Using keyword-based search only. Configure API key for better answers."
        )
    ...
    with col3:
        if st.button("Component affecting strength", use_container_width=True):
            ...
            if results:
                for r in results:
                    st.write(
                        f"- {r['component']} ({r['formula']}): {r['description'][:50]}..."
                    )
            else:
...
def render_laboratory():
    ...
    # Mode selection
    mode = st.radio(
        "Select Mode", ["Reasoning Experiment", "Hypothesis Generator"], horizontal=True
    )
    ...
    if mode == "Reasoning Experiment":
        ...
        if st.button("Analyze", type="primary"):
            ...
            with st.spinner("Reasoning through the problem..."):
                ...
                context = str(
                    [f"{d.get('label', n)} ({d.get('node_type')})" for n, d in nodes]
                )
        ...
        st.markdown(
            "Generate novel material compositions based on targets and constraints."
        )
            ...
            with st.spinner("Formulating hypothesis..."):
                ...
                if hypothesis:
                    ...
                    st.progress(
                        ...
                        text=f"Confidence Score: {hypothesis.confidence_score:.2f}",
                    )
                    ...
                    with st.expander("Show Reasoning Chain"):
                        for step in hypothesis.reasoning_chain:
                            st.markdown(
                                f"**{step.step_number}.** {step.thought} → *{step.conclusion}*"
                            )
...
def render_export():
    ...
    with col2:
        ...
        with st.expander("Preview Cypher"):
            st.code(
                (
                    cypher_statements[:2000] + "..."
                    if len(cypher_statements) > 2000
                    else cypher_statements
                ),
                language="cypher",
            )
...
def generate_cypher_export(kg: KnowledgeGraph) -> str:
    ...
    lines = [
        "// Concrete Knowledge Graph - Neo4j Import Script",
        "// Generated by Concrete KG App",
        "",
    ]
    ...
    for source, target, data in kg.graph.edges(data=True):
        ...
        props = {
            ...
            if k not in ["relationship_type", "color", "weight"]
        }
        ...
        lines.append(
            f'MATCH (s {{id: "{source}"}}), (t {{id: "{target}"}}) MERGE (s)-[r:{rel_type}{props_str}]->(t);'
        )
