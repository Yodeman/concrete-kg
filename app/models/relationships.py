...
class SubstitutionRelationship(LLMExportableModel):
    ...
    substitute: str = Field(..., description="Replacement component (e.g., 'Fly Ash')")
...
class SynergyRelationship(LLMExportableModel):
    ...
    component1: str = Field(..., description="First component name or formula")
    component2: str = Field(..., description="Second component name or formula")
    effect_type: Literal["SYNERGY", "ANTAGONISTIC"] = Field(
        ...,
        description="Type of interaction: 'SYNERGY' (positive) or 'ANTAGONISTIC' (negative)",
    )
    effect: str = Field(..., description="Description of the combined effect")
    ...
    mechanism: Optional[str] = Field(
        default=None, description="Mechanism explaining the interaction"
    )
    conditions: Optional[str] = Field(
        default=None, description="Conditions when this effect occurs"
    )
...
class SimilarityRelationship(LLMExportableModel):
    ...
    material1: str = Field(..., description="First material name")
    material2: str = Field(..., description="Second material name")
    similarity_score: float = Field(
        ..., ge=0, le=1, description="Similarity score (0-1, higher = more similar)"
    )
    ...
    shared_components: Optional[list] = Field(
        default=None, description="List of shared major components"
    )
