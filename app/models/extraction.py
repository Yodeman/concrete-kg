...
class ExtractionResult(LLMExportableModel):
    ...
    materials: List[MaterialComposition] = Field(
        default_factory=list, description="List of extracted material compositions"
    )
    relationships: List[ComponentRelationship] = Field(
        default_factory=list, description="General relationships between components"
    )

    # Enhanced extractions for material discovery
    substitutions: List[SubstitutionRelationship] = Field(
        default_factory=list, description="Material substitution relationships"
    )
    synergies: List[SynergyRelationship] = Field(
        default_factory=list, description="Synergistic and antagonistic effects"
    )
    quantitative_properties: List[QuantitativeProperty] = Field(
        default_factory=list, description="Quantitative property measurements"
    )

    # Metadata
    source_document: Optional[str] = Field(
        default=None, description="Source document title or identifier"
    )
